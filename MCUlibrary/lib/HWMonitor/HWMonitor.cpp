...
{
    if (_currentSensor >= HW_MAX_SENSORS) return;
...
{
    if (lastUpdate == 0) return UINT32_MAX;
...
const char* hwGetSensorName(uint8_t id)
{
    switch (id) {
        // CPU
        case SENSOR_CPU_TEMP:        return "CPU Temperature";
        case SENSOR_CPU_LOAD:       return "CPU Load";
        case SENSOR_CPU_CLOCK:      return "CPU Clock";
        case SENSOR_CPU_POWER:       return "CPU Power";
        case SENSOR_CPU_TEMP_CORE:  return "CPU Core Temp";
        case SENSOR_CPU_LOAD_CORE:  return "CPU Core Load";
        case SENSOR_CPU_POWER_CORE: return "CPU Core Power";
        case SENSOR_CPU_TEMP_CCD:    return "CPU CCD Temp";
        case SENSOR_CPU_VOLTAGE:    return "CPU Voltage";
        
        // GPU
        case SENSOR_GPU_TEMP:       return "GPU Temperature";
        case SENSOR_GPU_LOAD:       return "GPU Load";
        case SENSOR_GPU_CLOCK:       return "GPU Clock";
        case SENSOR_GPU_CLOCK_MEM:  return "GPU Memory Clock";
        case SENSOR_GPU_POWER:      return "GPU Power";
        case SENSOR_GPU_LOAD_MEM:   return "GPU Memory Load";
        case SENSOR_GPU_FAN:        return "GPU Fan";
        case SENSOR_GPU_TEMP_MEM:   return "GPU Memory Temp";
        case SENSOR_GPU_HOTSPOT:    return "GPU Hotspot";
        case SENSOR_GPU_LOAD_VIDEO:  return "GPU Video Load";
        
        // RAM
        case SENSOR_RAM_USED:       return "RAM Used";
        case SENSOR_RAM_AVAILABLE:   return "RAM Available";
        case SENSOR_RAM_LOAD:       return "RAM Load";
        
        // Disk
        case SENSOR_DISK_TEMP:      return "Disk Temperature";
        case SENSOR_DISK_LOAD:      return "Disk Load";
        case SENSOR_DISK_READ:      return "Disk Read";
        case SENSOR_DISK_WRITE:     return "Disk Write";
        
        // Network
        case SENSOR_NET_UP:         return "Network Upload";
        case SENSOR_NET_DOWN:       return "Network Download";
        
        // Motherboard
        case SENSOR_MB_TEMP:        return "Motherboard Temp";
        case SENSOR_MB_FAN1:        return "System Fan 1";
        case SENSOR_MB_FAN2:        return "System Fan 2";
        case SENSOR_MB_FAN3:        return "System Fan 3";
        case SENSOR_MB_VOLTAGE:      return "System Voltage";
        
        // Battery
        case SENSOR_BATTERY_LEVEL:  return "Battery Level";
        case SENSOR_BATTERY_VOLTAGE: return "Battery Voltage";
        case SENSOR_BATTERY_RATE:   return "Battery Rate";
...
{
    if (id >= 0x01 && id <= 0x0F) return "CPU";
    if (id >= 0x10 && id <= 0x1F) return "GPU";
    if (id >= 0x20 && id <= 0x2F) return "RAM";
    if (id >= 0x30 && id <= 0x3F) return "Disk";
    if (id >= 0x40 && id <= 0x4F) return "Network";
    if (id >= 0x50 && id <= 0x5F) return "Motherboard";
    if (id >= 0x60 && id <= 0x6F) return "Battery";
    if (id >= 0x80 && id <= 0xFE) return "Custom";
    return "Unknown";
}